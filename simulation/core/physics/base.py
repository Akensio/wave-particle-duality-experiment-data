...
class DiffractionPhysics:
    ...
    def __init__(self, grating_spacing, screen_width=None, distance_to_screen=None):
    ...
    def calculate_intensity_pattern(self, wavelength, num_points=DEFAULT_NUM_POINTS, num_slits=5):
        ...
        intensity_factor = np.divide(numerator, denominator, out=np.zeros_like(numerator), where=denominator > 1e-10)

        # Normalize intensity
        intensity = intensity_factor / np.max(intensity_factor) if np.max(intensity_factor) > 0 else intensity_factor

        return angles, intensity

    def _calculate_large_slit_pattern(self, wavelength, angles, num_slits):
        """
